...
// API géo officielle de l'État : nom de ville -> code INSEE (gratuite, sans clé)
async function resolveCity(name) {
  const url =
    "https://geo.api.gouv.fr/communes?nom=" +
    encodeURIComponent(name) +
    "&fields=nom,code,codesPostaux,population&boost=population&limit=1";
  const r = await fetch(url);
  if (!r.ok) throw new Error("géo HTTP " + r.status);
  const arr = await r.json();
  if (!arr.length) throw new Error("ville introuvable : " + name);
  return { insee: arr[0].code, nom: arr[0].nom };
}
...
async function fetchFranceTravail(keyword, communeInsee) {
...
async function fetchMeteojobPage(keyword, page, city) {
  const params = new URLSearchParams({ where: city });
...
async function fetchMeteojob(keyword, city) {
  let all = [];
  for (let p = 1; p <= MJ.pages; p++) {
    const page = await fetchMeteojobPage(keyword, p, city);
...
async function fetchAdzuna(keyword, city) {
...
export default async function handler(req, res) {
  ...
  const ville = (url.searchParams.get("ville") || DEFAULT_CITY).trim() || DEFAULT_CITY;
  ...
  let geo = null;
  if (wantFT) {
    try { geo = await resolveCity(ville); }
    catch (e) { errors.push("Localisation : " + e.message); }
  }
  let ft = [];
  let mj = [];
  let adz = [];
  ...
  await Promise.all([
    wantFT && geo
      ? fetchFranceTravail(keyword, geo.insee)
          .then((r) => (ft = r))
          .catch((e) => errors.push("France Travail : " + e.message))
      : null,
    wantMJ
      ? fetchMeteojob(keyword, ville)
          .then((r) => (mj = r))
          .catch((e) => errors.push("Meteojob : " + e.message))
      : null,
    wantADZ
      ? fetchAdzuna(keyword, ville)
          .then((r) => (adz = r))
