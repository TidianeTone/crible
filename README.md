...
En ligne : <https://cvcrible.vercel.app>
...
le tout via l'API DeepSeek (la lecture du CV en PDF reste chez Claude). Front en HTML/CSS/JS
sans framework, back en fonctions serverless Vercel.
...
Aucune clé ne figure dans le code. Les cinq premières sont obligatoires côté serveur :

| Variable | Où l'obtenir | Sert à |
|---|---|---|
| `ANTHROPIC_API_KEY` | [console.anthropic.com](https://console.anthropic.com) | Lecture du CV en PDF (toujours), et tout le reste si `IA_PROVIDER=anthropic` |
| `DEEPSEEK_API_KEY` | [platform.deepseek.com](https://platform.deepseek.com) | Classement, lettres, adaptation — fournisseur par défaut |
...
**Choix du fournisseur d'IA** — variable facultative `IA_PROVIDER` :

| Valeur | Effet |
|---|---|
| absente, ou `deepseek` | DeepSeek V4 Flash (tri, analyse) et V4 Pro (réécriture du CV) — **essai en cours** |
| `anthropic` | Retour à Claude Haiku / Sonnet, comme avant |

La lecture du CV en PDF (`/api/profile`) reste sur Claude dans les deux cas : DeepSeek ne sait
pas lire un PDF envoyé en pièce jointe. `ANTHROPIC_API_KEY` reste donc nécessaire.
...
styles.css      thème « rapport d'analyse » (fond graphite, rampe de mesure unique)
api/
  _claude.js    helpers partagés (appel Anthropic ou DeepSeek, parsing JSON, langue)
  search.js     agrégation France Travail + Adzuna + Meteojob
  profile.js    CV PDF -> profil + expériences structurées   (Claude Haiku, toujours)
  match.js      notation des offres selon le profil          (rapide)
  letter.js     brouillon de lettre de motivation            (rapide)
  tailor.js     adaptation du CV à une offre, en 3 étapes    (rapide + soigné)
```
...
2. **Réécriture XYZ** — les expériences sont réécrites selon la formule de Google
   ...
   les signaux d'alerte. C'est l'étape confiée au modèle le plus soigné, là où la qualité rédactionnelle compte.
...
Le PDF du CV n'est stocké nulle part. Il est transmis une seule fois à l'API Claude (jamais à DeepSeek) pour en
